...
namespace ProtoBuf.Serializers
{
    sealed class TypeSerializer : IProtoTypeSerializer
    {
        ...
        public TypeSerializer(TypeModel model, Type forType, int[] fieldNumbers, IProtoSerializer[] serializers, MethodInfo[] baseCtorCallbacks, bool isRootType, bool useConstructor, CallbackSet callbacks, Type constructType, MethodInfo factory)
        {
            ...
            SortProtoSerializers(fieldNumbers, serializers);
...
#else
                return (forType.IsClass || forType.IsInterface);
#endif
            }
        }
        ...
        public static void SortProtoSerializers(int[] keys, IProtoSerializer[] values)
        {
            ...
            do
            {
                ...
                for (int i = 1; i < keys.Length; i++)
                {
                    if (keys[i - 1] > keys[i])
                    {
                        ...
                        keys[i] = keys[i - 1];
                        keys[i - 1] = tmpKey;
                        IProtoSerializer tmpValue = values[i];
                        values[i] = values[i - 1];
                        values[i - 1] = tmpValue;
                        swapped = true;
                    }
