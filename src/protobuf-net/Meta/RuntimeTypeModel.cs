...
namespace ProtoBuf.Meta
{
    ...
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        protected internal override object Deserialize(int key, object value, ProtoReader source)
        {
            ...
            MetaType metaType = (MetaType)types[key];
            //Debug.WriteLine("This is the Type: "+metaType.Type.FullName);
            IProtoSerializer ser = metaType.Serializer;
            if (value == null && Helpers.IsValueType(ser.ExpectedType))
            {
                if (ser.RequiresOldValue)
                {
                    object newInstance = Activator.CreateInstance(ser.ExpectedType);
                    value = newInstance;
                }
...
#else
                if (type.IsClass && Helpers.GetConstructor(type, Helpers.EmptyTypes, true) != null)
#endif
                {
                    defaultType = type;
                }
